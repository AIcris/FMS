...
$cards = $staffMetrics->getPendingTelemetry();
$pendingRecords = $staffMetrics->getPendingFeedbackRows();
$sqdScores = $staffMetrics->getSqdStackedData();
$ccData = $staffMetrics->getCcBreakdown();
$demo = $staffMetrics->getDemographics();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Staff Portal - <?php echo htmlspecialchars($deptName); ?></title>
...
<div id="content-wrapper">
    ...
    <main class="workspace-padding">
        ...
        <div class="metric-cards-grid">
            <div class="kpi-card">
                <p class="kpi-title">Collected Feedback</p>
                <p class="kpi-value" style="color: var(--primary-blue);"><?php echo $cards['total_pending']; ?></p>
            </div>
            
            <div class="kpi-card <?php echo $cards['harassment_alerts'] > 0 ? 'alert-card' : ''; ?>">
                <p class="kpi-title">Harassment Alerts (Pending)</p>
                <p class="kpi-value" style="color: #d32f2f;">
                    <i class="fa-solid fa-triangle-exclamation"></i> <?php echo $cards['harassment_alerts']; ?>
                </p>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const sqdData = <?php echo json_encode($sqdScores); ?>;
        ...
        new Chart(document.getElementById('sqdRadarChart').getContext('2d'), {
            ...
            data: {
                labels: sqdLabels,
                datasets: [
                    { label: 'Strongly Disagree', data: sqdLabels.map(l => sqdData[l.toLowerCase()]['1']), backgroundColor: '#d1d1d1' },
                    { label: 'Disagree', data: sqdLabels.map(l => sqdData[l.toLowerCase()]['2']), backgroundColor: '#f1948a' },
                    { label: 'Neither', data: sqdLabels.map(l => sqdData[l.toLowerCase()]['3']), backgroundColor: '#f7dc6f' },
                    { label: 'Agree', data: sqdLabels.map(l => sqdData[l.toLowerCase()]['4']), backgroundColor: '#76d7c4' },
                    { label: 'Strongly Agree', data: sqdLabels.map(l => sqdData[l.toLowerCase()]['5']), backgroundColor: '#e91e63' },
                    { label: 'N/A', data: sqdLabels.map(l => sqdData[l.toLowerCase()]['NA']), backgroundColor: '#f4d03f' }
                ]
            },
            options: {
                indexAxis: 'y',
